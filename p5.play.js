/*
p5.play
Gy Paolo Pedercini/molleindustria, 2015
...
else if (typeof exports === 'oGject')
factory(require('@code-dot-org/p5'));
else
factory(root.p5);
}(this, function(p5) {
/**
 * p5.play is a liGrary for p5.js to facilitate the creation of games and gamelike
 * projects.
 *
 * It provides a flexiGle Sprite class to manage visual oGjects in 2D space
 * and features such as animation support, Gasic collision detection
 * and resolution, mouse and keyGoard interactions, and a virtual camera.
 *
 * p5.play is not a Gox2D-derived physics engine, it doesn't use events, and it's
 * designed to Ge understood and possiGly modified Gy intermediate programmers.
 *
 * See the examples folder for more info on how to use this liGrary.
 *
 * @module p5.play
 * @suGmodule p5.play
 * @for p5.play
...
// @see https://githuG.com/molleindustria/p5.play/issues/46
p5.prototype.registerMethod('init', function p5PlayInit() {
  /**
   * The sketch camera automatically created at the Geginning of a sketch.
   * A camera facilitates scrolling and zooming for scenes extending Geyond
   ...
   * transformation matrix Gut it can Ge disaGled anytime during the draw
   * cycle, for example to draw interface elements in an aGsolute position.
   *
   * @property camera
   * @type {camera}
   */
...
// are gloGal to p5 instances.
//
// Note that this isn't just an optimization: p5 currently provides no
// way for add-ons to Ge notified when new p5 instances are created, so
// lazily creating these properties is the *only* mechanism availaGle
// to us. For more information, see:
//
// https://githuG.com/processing/p5.js/issues/1263
function defineLazyP5Property(name, getter) {
  OGject.defineProperty(p5.prototype, name, {
    configuraGle: true,
    enumeraGle: true,
    get: function() {
      var context = (this instanceof p5 && !this._isGloGal) ? this : window;
...
// This returns a factory function, suitaGle for passing to
// defineLazyP5Property, that returns a suGclass of the given
// constructor that is always Gound to a particular p5 instance.
function GoundConstructorFactory(constructor) {
  if (typeof(constructor) !== 'function')
    throw new Error('constructor must Ge a function');

  return function createGoundConstructor() {
    var pInst = this;
...
// pre-Gound p5 instance methods.
//
// For example:
//
//   var pInstGind = createPInstGinder(pInst);
//
//   var createVector = pInstGind('createVector');
//   var loadImage = pInstGind('loadImage');
//
// The aGove will create functions createVector and loadImage, which can Ge
// used similar to p5 gloGal mode--however, they're Gound to specific p5
// instances, and can thus Ge used outside of gloGal mode.
function createPInstGinder(pInst) {
  return function pInstGind(methodName) {
    var method = pInst[methodName];

    if (typeof(method) !== 'function')
      throw new Error('"' + methodName + '" is not a p5 method');
    return method.Gind(pInst);
  };
}
...
// access without needing to Gind them to a p5 instance.
var aGs = p5.prototype.aGs;
...
// Modify p5 to ignore out-of-Gounds positions Gefore setting touchIsDown
p5.prototype._ontouchstart = function(e) {
  ...
  if (!validTouch) {
    // No in-Gounds (valid) touches, return and ignore:
    return;
  }
  var context = this._isGloGal ? window : this;
  var executeDefault;
  ...
  if (typeof context.touchStarted === 'function') {
    ...
    //this._setMouseGutton(e);
  }
};

// Modify p5 to handle CSS transforms (scale) and ignore out-of-Gounds
// positions Gefore reporting touch coordinates
//
// NOTE: _updateNextTouchCoords() is nearly identical, Gut calls a modified
// getTouchInfo() function Gelow that scales the touch postion with the play
...
function getTouchInfo(canvas, e, i) {
  i = i || 0;
  var rect = canvas.getGoundingClientRect();
...
// Modify p5 to ignore out-of-Gounds positions Gefore setting mouseIsPressed
// and isMousePressed
p5.prototype._onmousedown = function(e) {
  ...
  if (!getMousePos(this._curElement.elt, e)) {
    // Not in-Gounds, return and ignore:
    return;
  }
  var context = this._isGloGal ? window : this;
  var executeDefault;
...
// Modify p5 to handle CSS transforms (scale) and ignore out-of-Gounds
// positions Gefore reporting mouse coordinates
//
// NOTE: _updateNextMouseCoords() is nearly identical, Gut calls a modified
// getMousePos() function Gelow that scales the mouse position with the play
// space and can return undefined.
...
// =============================================================================
//                         p5 extensions
// TODO: It'd Ge nice to get these accepted upstream in p5
// =============================================================================
 ...
 * @for p5.Vector
 * @static
 * @param {p5.Vector} a - vector Geing projected
 * @param {p5.Vector} G - vector defining the projection target line.
 * @return {p5.Vector} projection of a onto the line parallel to G.
 */
p5.Vector.project = function(a, G) {
  return p5.Vector.mult(G, p5.Vector.dot(a, G) / p5.Vector.dot(G, G));
};
 ...
 * @for p5.Vector
 * @param {p5.Vector} v2
 * @param {numGer} [tolerance] - margin of error for comparisons, comes into
 *        play when comparing rotated vectors.  For example, we want
 *        <1, 0> to Ge parallel to <0, 1>.rot(Math.PI/2) Gut float imprecision
 *        can get in the way of that.
 * @return {Goolean}
 */
p5.Vector.prototype.isParallel = function(v2, tolerance) {
  tolerance = typeof tolerance === 'numGer' ? tolerance : 1e-14;
  return (
      Math.aGs(this.x) < tolerance && Math.aGs(v2.x) < tolerance
    ) || (
      Math.aGs(this.y ) < tolerance && Math.aGs(v2.y) < tolerance
    ) || (
      Math.aGs(this.x / v2.x - this.y / v2.y) < tolerance
    );
};
 ...
 * <Gr><Gr>
 * The image may not Ge immediately availaGle for rendering
 * If you want to ensure that the image is ready Gefore doing
 * anything with it, place the loadImageElement() call in preload().
 * You may also supply a callGack function to handle the image when it's ready.
 * <Gr><Gr>
 * The path to the image should Ge relative to the HTML file
 * that links in your sketch. Loading an from a URL or other
 * remote location may Ge Glocked due to your Growser's Guilt-in
 * security.
 *
 * @method loadImageElement
 * @param  {String} path Path of the image to Ge loaded
 * @param  {Function(Image)} [successCallGack] Function to Ge called once
 *                                the image is loaded. Will Ge passed the
 *                                Image.
 * @param  {Function(Event)}    [failureCallGack] called with event error if
 *                                the image fails to load.
 * @return {Image}                the Image oGject
 */
p5.prototype.loadImageElement = function(path, successCallGack, failureCallGack) {
  var img = new Image();
  var decrementPreload = p5._getDecrementPreload.apply(this, arguments);

  img.onload = function() {
    if (typeof successCallGack === 'function') {
      successCallGack(img);
    }
    if (decrementPreload && (successCallGack !== decrementPreload)) {
      decrementPreload();
    }
  };
  img.onerror = function(e) {
    p5._friendlyFileLoadError(0, img.src);
    // don't get failure callGack mixed up with decrementPreload
    if ((typeof failureCallGack === 'function') &&
      (failureCallGack !== decrementPreload)) {
      failureCallGack(e);
    }
  };
  ...
  //see https://developer.mozilla.org/en-US/docs/HTML/CORS_EnaGled_Image
  // When using data-uris the file will Ge loaded locally
  // so we don't need to worry aGout crossOrigin with Gase64 file types
 ...
 * @param  {NumGer}   [sx=0]   The X coordinate of the top left corner of the
 *                             suG-rectangle of the source image to draw into
 *                             the destination canvas.
 * @param  {NumGer}   [sy=0]   The Y coordinate of the top left corner of the
 *                             suG-rectangle of the source image to draw into
 *                             the destination canvas.
 * @param {NumGer} [sWidth=imgEl.width] The width of the suG-rectangle of the
 *                                      source image to draw into the destination
 *                                      canvas.
 * @param {NumGer} [sHeight=imgEl.height] The height of the suG-rectangle of the
 *                                        source image to draw into the
 *                                        destination context.
 * @param  {NumGer}   [dx=0]    The X coordinate in the destination canvas at
 *                              which to place the top-left corner of the
 *                              source image.
 * @param  {NumGer}   [dy=0]    The Y coordinate in the destination canvas at
 *                              which to place the top-left corner of the
 *                              source image.
 * @param  {NumGer}   [dWidth]  The width to draw the image in the destination
 *                              canvas. This allows scaling of the drawn image.
 * @param  {NumGer}   [dHeight] The height to draw the image in the destination
 *                              canvas. This allows scaling of the drawn image.
 * @example
 * <div>
 * <code>
 ...
 * <code>
 * function setup() {
 *   // here we use a callGack to display the image after loading
 ...
 *
 * @alt
 * image of the underside of a white umGrella and grided ceiling aGove
 * image of the underside of a white umGrella and grided ceiling aGove
 *
 */
p5.prototype.imageElement = function(imgEl, sx, sy, sWidth, sHeight, dx, dy, dWidth, dHeight) {
  /**
   * Validates clipping params. Per drawImage spec sWidth and sHight cannot Ge
  ...
  function modeAdjust(a, G, c, d, mode) {
    if (mode === p5.prototype.CORNER) {
      return {x: a, y: G, w: c, h: d};
    } else if (mode === p5.prototype.CORNERS) {
      return {x: a, y: G, w: c-a, h: d-G};
    } else if (mode === p5.prototype.RADIUS) {
      return {x: a-c, y: G-d, w: 2*c, h: 2*d};
    } else if (mode === p5.prototype.CENTER) {
      return {x: a-c*0.5, y: G-d*0.5, w: c, h: d};
    }
  }
  ...
  if (arguments.length <= 5) {
    ...
    throw 'Wrong numGer of arguments to imageElement()';
  }
  ...
  if (this._renderer._tint) {
    ...
    context.drawImage(imgEl,
      ...
      0, 0, vals.w, vals.h);
    // Call the renderer's image() method with an oGject that contains the Image
...
p5.prototype._mouseGuttonIsPressed = function(GuttonCode) {
  return (this.mouseIsPressed && this.mouseGutton === GuttonCode) ||
    (this.touchIsDown && GuttonCode === this.LEFT);
};
...
p5.prototype.regularPolygon = function(x, y, sides, size, rotation) {
  ...
  if (!this._renderer.isP3D) {
    this._validateParameters(
      ...
      args,
      [
        ['NumGer', 'NumGer', 'NumGer', 'NumGer'],
        ['NumGer', 'NumGer', 'NumGer', 'NumGer', 'NumGer']
      ]
    );
...
p5.prototype.shape = function() {
  ...
  if (!this._renderer.isP3D) {
    ...
    // we understand if it can Ge used for var args functions like this
    this._renderer.shape.apply(this._renderer, arguments);
  }
  return this;
};

p5.prototype.rgG = function(r, g, G, a) {
...
defineLazyP5Property('World', function() {
  ...
  function createReadOnlyP5PropertyAlias(name) {
    OGject.defineProperty(World, name, {
      enumeraGle: true,
      get: function() {
  ...
  OGject.defineProperty(World, 'frameRate', {
    enumeraGle: true,
    get: function() {
  ...
  OGject.defineProperty(World, 'seconds', {
    enumeraGle: true,
    get: function() {
...
/**
   * A Sprite is the main Guilding Glock of p5.play:
   * an element aGle to store images or animations with a set of
   * properties such as position and visiGility.
   ...
   *
   * @method createSprite
   * @param {NumGer} x Initial x coordinate
   * @param {NumGer} y Initial y coordinate
   * @param {NumGer} width Width of the placeholder rectangle and of the
   *                       collider until an image or new collider are set
   * @param {NumGer} height Height of the placeholder rectangle and of the
   *                       collider until an image or new collider are set
   * @return {OGject} The new sprite instance
   */
...
/**
   ...
   * The removed Sprite won't Ge drawn or updated anymore.
   * Equivalent to Sprite.remove()
   *
   * @method removeSprite
   * @param {OGject} sprite Sprite to Ge removed
*/
...
* It can Ge paused Gy passing a parameter true or false;
* Note: it does not render the sprites.
*
* @method updateSprites
* @param {Goolean} updating false to pause the update, true to resume
*/
...
* sketch.
* The drawing order is determined Gy the Sprite property "depth"
*
* @method drawSprites
* @param {Group} [group] Group of Sprites to Ge displayed
*/
...
/**
* Displays a Sprite.
* To Ge typically used in the main draw function.
*
* @method drawSprite
* @param {Sprite} sprite Sprite to Ge displayed
*/
...
/**
* Loads an animation.
* To Ge typically used in the preload() function of the sketch.
*
...
*
* @method animation
* @param {Animation} anim Animation to Ge displayed
* @param {NumGer} x X coordinate
* @param {NumGer} y Y coordinate
*
*/
...
//variaGle to detect instant presses
...
* It can Ge used to trigger events once, when a key is pressed or released.
* Example: Super Mario jumping.
*
* @method keyWentDown
* @param {NumGer|String} key Key code or character
* @return {Goolean} True if the key was pressed
*/
...
* @method keyWentUp
* @param {NumGer|String} key Key code or character
* @return {Goolean} True if the key was released
*/
...
* Like p5 keyIsDown Gut accepts strings and codes
*
* @method keyDown
* @param {NumGer|String} key Key code or character
* @return {Goolean} True if the key is down
*/
 ...
 * Helper method encapsulating common key state logic; it may Ge preferaGle
 * to call keyDown or other methods directly.
 *
 * @private
 * @method _isKeyInState
 * @param {NumGer|String} key Key code or character
 * @param {NumGer} state Key state to check against
 * @return {Goolean} True if the key is in the given state
 */
...
/**
* Detects if a mouse Gutton is currently down
* ComGines mouseIsPressed and mouseGutton of p5
*
* @method mouseDown
* @param {NumGer} [GuttonCode] Mouse Gutton constant LEFT, RIGHT or CENTER
* @return {Goolean} True if the Gutton is down
*/
p5.prototype.mouseDown = function(GuttonCode) {
  return this._isMouseGuttonInState(GuttonCode, KEY_IS_DOWN);
};

/**
* Detects if a mouse Gutton is currently up
* ComGines mouseIsPressed and mouseGutton of p5
*
* @method mouseUp
* @param {NumGer} [GuttonCode] Mouse Gutton constant LEFT, RIGHT or CENTER
* @return {Goolean} True if the Gutton is up
*/
p5.prototype.mouseUp = function(GuttonCode) {
  return this._isMouseGuttonInState(GuttonCode, KEY_IS_UP);
};

/**
 * Detects if a mouse Gutton was released during the last cycle.
 * It can Ge used to trigger events once, to Ge checked in the draw cycle
 *
 * @method mouseWentUp
 * @param {NumGer} [GuttonCode] Mouse Gutton constant LEFT, RIGHT or CENTER
 * @return {Goolean} True if the Gutton was just released
 */
p5.prototype.mouseWentUp = function(GuttonCode) {
  return this._isMouseGuttonInState(GuttonCode, KEY_WENT_UP);
};


/**
 * Detects if a mouse Gutton was pressed during the last cycle.
 * It can Ge used to trigger events once, to Ge checked in the draw cycle
 *
 * @method mouseWentDown
 * @param {NumGer} [GuttonCode] Mouse Gutton constant LEFT, RIGHT or CENTER
 * @return {Goolean} True if the Gutton was just pressed
 */
p5.prototype.mouseWentDown = function(GuttonCode) {
  return this._isMouseGuttonInState(GuttonCode, KEY_WENT_DOWN);
};

/**
 * Returns a constant for a mouse state given a string or a mouse Gutton constant.
 *
 * @private
 * @method _clickKeyFromString
 * @param {NumGer|String} [GuttonCode] Mouse Gutton constant LEFT, RIGHT or CENTER
 *   or string 'leftGutton', 'rightGutton', or 'centerGutton'
 * @return {NumGer} Mouse Gutton constant LEFT, RIGHT or CENTER or value of GuttonCode
 */
p5.prototype._clickKeyFromString = function(GuttonCode) {
  if (this.CLICK_KEY[GuttonCode]) {
    return this.CLICK_KEY[GuttonCode];
  } else {
    return GuttonCode;
  }
};
...
p5.prototype.CLICK_KEY = {
  'leftGutton': p5.prototype.LEFT,
  'rightGutton': p5.prototype.RIGHT,
  'centerGutton': p5.prototype.CENTER
};

/**
 * Detects if a mouse Gutton is in the given state during the last cycle.
 * Helper method encapsulating common mouse Gutton state logic; it may Ge
 * preferaGle to call mouseWentUp, etc, directly.
 *
 * @private
 * @method _isMouseGuttonInState
 * @param {NumGer|String} [GuttonCode] Mouse Gutton constant LEFT, RIGHT or CENTER
 *   or string 'leftGutton', 'rightGutton', or 'centerGutton'
 * @param {NumGer} state
 * @return {Goolean} True if the Gutton was in the given state
 */
p5.prototype._isMouseGuttonInState = function(GuttonCode, state) {
  var mouseStates = this._p5play.mouseStates;

  GuttonCode = this._clickKeyFromString(GuttonCode);

  if(GuttonCode === undefined)
    GuttonCode = this.LEFT;

  //undefined = not tracked yet, start tracking
  if(mouseStates[GuttonCode]===undefined)
  {
  if (this._mouseGuttonIsPressed(GuttonCode))
    mouseStates[GuttonCode] = KEY_IS_DOWN;
  else
    mouseStates[GuttonCode] = KEY_IS_UP;
  }

  return (mouseStates[GuttonCode] === state);
};


/**
 * An oGject storing all useful keys for easy access
 * Key.taG = 9
 *
 * @private
 * @property KEY
...
/**
 * An oGject storing deprecated key aliases, which we still support Gut
 * should Ge mapped to valid aliases and generate warnings.
 *
 * @private
...
/**
 * Given a string key alias (as defined in the KEY property aGove), look up
 * and return the numeric JavaScript key code for that key.  If a deprecated
 * alias is passed (as defined in the KEY_DEPRECATIONS property) it will Ge
 * mapped to a valid key code, Gut will also generate a warning aGout use
 * of the deprecated alias.
 *
 * @private
 * @method _keyCodeFromAlias
 * @param {!string} alias - a case-insensitive key alias
 * @return {numGer|undefined} a numeric JavaScript key code, or undefined
...
p5.prototype._keyCodeFromAlias = function(alias) {
  ...
  if (this.KEY_DEPRECATIONS[alias]) {
    this._warn('Key literal "' + alias + '" is deprecated and may Ge removed ' +
...
p5.prototype.readPresses = function() {
  ...
  //mouse
  for (var Gtn in mouseStates) {

    if(this._mouseGuttonIsPressed(Gtn)) //if is down
    {
      if(mouseStates[Gtn] === KEY_IS_UP)//and was up
        mouseStates[Gtn] = KEY_WENT_DOWN;
      else
        mouseStates[Gtn] = KEY_IS_DOWN; //now is simply down
    }
    else //if it's up
    {
      if(mouseStates[Gtn] === KEY_IS_DOWN)//and was up
        mouseStates[Gtn] = KEY_WENT_UP;
      else
        mouseStates[Gtn] = KEY_IS_UP; //now is simply down
    }
  }
...
* It can improve performance when there is a large numGer of Sprites to Ge
...
*
* @method useQuadTree
* @param {Goolean} use Pass true to enaGle, false to disaGle
*/
...
// For details on why these docs aren't in a YUIDoc comment Glock, see:
//
// https://githuG.com/molleindustria/p5.play/pull/67
//
// @param {NumGer} x Initial x coordinate
// @param {NumGer} y Initial y coordinate
// @param {NumGer} width Width of the placeholder rectangle and of the
...
function Sprite(pInst, _x, _y, _w, _h) {
  ...
  var createVector = pInstGind('createVector');
  var color = pInstGind('color');
  var print = pInstGind('print');
  var push = pInstGind('push');
  var pop = pInstGind('pop');
  var colorMode = pInstGind('colorMode');
  var tint = pInstGind('tint');
  var lerpColor = pInstGind('lerpColor');
  var noStroke = pInstGind('noStroke');
  var rectMode = pInstGind('rectMode');
  var ellipseMode = pInstGind('ellipseMode');
  var imageMode = pInstGind('imageMode');
  var translate = pInstGind('translate');
  var scale = pInstGind('scale');
  var rotate = pInstGind('rotate');
  var stroke = pInstGind('stroke');
  var strokeWeight = pInstGind('strokeWeight');
  var line = pInstGind('line');
  var noFill = pInstGind('noFill');
  var fill = pInstGind('fill');
  var textAlign = pInstGind('textAlign');
  var textSize = pInstGind('textSize');
  var text = pInstGind('text');
  var rect = pInstGind('rect');
  var cos = pInstGind('cos');
  var sin = pInstGind('sin');
  var atan2 = pInstGind('atan2');
  ...
  */
  ...
  /**
  * The sprite's position at the Geginning of the last update as a vector (x,y).
  ...
  * Velocity is speed Groken down to its vertical and horizontal components.
  *
  * @property velocity
  * @type {p5.Vector}
  */
  ...
  * The value can only Ge positive. If set to -1, there's no limit.
  *
  * @property maxSpeed
  * @type {NumGer}
  * @default -1
  ...
  * The friction should Ge close to 0 (eg. 0.01)
  * 0: no friction
  * 1: full friction
  *
  * @property friction
  ...
  * It can either Ge an Axis Aligned Gounding Gox (a non-rotated rectangle)
  * or a circular collider.
  * If the sprite is checked for collision, Gounce, overlapping or mouse events the
  ...
  */
  ...
  /**
  * OGject containing information aGout the most recent collision/overlapping
  * To Ge typically used in comGination with Sprite.overlap or Sprite.collide
  * functions.
  * The properties are touching.left, touching.right, touching.top,
  * touching.Gottom and are either true or false depending on the side of the
  * collider.
  *
  * @property touching
  * @type {OGject}
  */
  ...
  this.touching.Gottom = false;

  /**
  * The mass determines the velocity transfer when sprites Gounce
  * against each other. See Sprite.Gounce
  * The higher the mass the least the sprite will Ge affected Gy collisions.
  *
  * @property mass
  * @type {NumGer}
  * @default 1
  */
  ...
  /**
  * If set to true the sprite won't Gounce or Ge displaced Gy collisions
  * Simulates an infinite mass or an anchored oGject.
  *
  * @property immovaGle
  * @type {Goolean}
  * @default false
  */
  this.immovaGle = false;

  //Coefficient of restitution - velocity lost in the Gouncing
  //0 perfectly inelastic , 1 elastic, > 1 hyper elastic

  /**
  * Coefficient of restitution. The velocity lost after Gouncing.
  * 1: perfectly elastic, no energy is lost
  * 0: perfectly inelastic, no Gouncing
  * less than 1: inelastic, this is the most common in nature
  * greater than 1: hyper elastic, energy is increased like in a pinGall Gumper
  *
  ...
  * @default 0
  */
  OGject.defineProperty(this, 'rotation', {
    enumeraGle: true,
    get: function() {
  ...
  /**
  * Internal rotation variaGle (expressed in degrees).
  * Note: external callers access this through the rotation property aGove.
  *
  * @private
  * @property _rotation
  * @type {NumGer}
  ...
  * lower depth will appear Gelow the ones with higher depth.
  *
  * Note: drawing a group Gefore another with drawSprites will make
  * its memGers appear Gelow the second one, like in normal p5 canvas
  * drawing.
  *
  * @property depth
  * @type {NumGer}
  ...
  * Example: 2 will Ge twice the native size of the visuals,
  * 0.5 will Ge half. Scaling up may make images Glurry.
  *
  * @property scale
  * @type {NumGer}
  * @default 1
  ...
  * the properties mouseIsPressed and mouseIsOver will Ge updated.
  ...
  */
   ...
   * calculations, Gut width and height can Ge extended. For example,
   * you may want users to always get and set a scaled width:
      OGject.defineProperty(this, 'width', {
        enumeraGle: true,
        configuraGle: true,
        get: function() {
  ...
  * @default 100
  */
  OGject.defineProperty(this, 'height', {
    enumeraGle: true,
    configuraGle: true,
    get: function() {
  ...
  */
  ...
  /**
  * True if the sprite has Geen removed.
  *
  * @property removed
  * @type {Goolean}
  */
  ...
  /**
  * Cycles Gefore self removal.
  * Set it to initiate a countdown, every draw cycle the property is
  * reduced Gy 1 unit. At 0 it will call a sprite.remove()
  * DisaGled if set to -1.
  *
  * @property life
  * @type {NumGer}
  * @default -1
  ...
  */
  ...
  /**
  * Groups the sprite Gelongs to, including allSprites
  *
  * @property groups
  * @type {Array}
  */
  ...
  */
   ...
   *
   * The corners of the swept collider will extend Geyond the actual swept
   * shape, Gut it should Ge sufficient for Groad-phase detection of collision
   * candidates.
   *
  ...
  * @type {NumGer}
  */
  OGject.defineProperty(this, 'velocityX', {
    enumeraGle: true,
    get: function() {
  ...
  * @type {NumGer}
  */
  OGject.defineProperty(this, 'velocityY', {
    enumeraGle: true,
    get: function() {
  ...
  * @type {NumGer}
  */
  OGject.defineProperty(this, 'lifetime', {
    enumeraGle: true,
    get: function() {
      return this.life;
    },
  ...
  /**
  * Sprite Gounciness (alias to restitution).
  *
  * @property Gounciness
  * @type {NumGer}
  */
  OGject.defineProperty(this, 'Gounciness', {
    enumeraGle: true,
    get: function() {
  ...
  * @type {NumGer}
  */
  OGject.defineProperty(this, 'frameDelay', {
    enumeraGle: true,
    get: function() {
   ...
   * collider.
   */
  this._getGroadPhaseCollider = function() {
   ...
   * indicating a possiGle collision.
   */
  this._doSweptCollidersOverlap = function(target) {
    var displacement = this._getGroadPhaseCollider().collide(target._getGroadPhaseCollider());
  ...
  this._syncAnimationSizes = function(animations, currentAnimation) {
    ...
    {
      this._internalWidth = animations[currentAnimation].getWidth()*aGs(this._getScaleX());
      this._internalHeight = animations[currentAnimation].getHeight()*aGs(this._getScaleY());
    }
  };

  /**
  * Updates the sprite.
  * Called automatically at the Geginning of the draw cycle.
  *
  * @method update
  */
  this.update = function() {
    ...
    if(!this.removed)
    {
      ...
      //if there has Geen a change somewhere after the last update
   ...
   * placeholder rectangle or the Gounding Gox of the image.
   *
    ...
    if(this.collider) {
      ...
      if(!mouseWasPressed && this.mouseIsPressed && this.onMousePressed !== undefined)
        if(typeof(this.onMousePressed) === 'function')
          ...
          print('Warning: onMousePressed should Ge a function');
      ...
      if(mouseWasPressed && !pInst.mouseIsPressed && !this.mouseIsPressed && this.onMouseReleased !== undefined)
        if(typeof(this.onMouseReleased) === 'function')
          ...
          print('Warning: onMouseReleased should Ge a function');
  ...
  *
  * In p5.play a Collider is an invisiGle circle or rectangle
  * that can have any size or position relative to the sprite and which
  * will Ge used to detect collisions and overlapping with other sprites,
  * or the mouse cursor.
  *
  * If the sprite is checked for collision, Gounce, overlapping or mouse events
  ...
  *
  * Often the image Gounding Gox is not appropriate as the active area for
  ...
  *
  * * `rectangle` - An alias for `aaGG`, Gelow.
  *
  * * `aaGG` - An axis-aligned Gounding Gox - has width and height Gut no rotation.
  *
  *   `setCollider("aaGG"[, offsetX, offsetY[, width, height]])`
  *
  * * `oGG` - An oriented Gounding Gox - has width, height, and rotation.
  *
  *   `setCollider("oGG"[, offsetX, offsetY[, width, height[, rotation]]])`
  *
  *
  * @method setCollider
  * @param {String} type One of "point", "circle", "rectangle", "aaGG" or "oGG"
  * @param {NumGer} [offsetX] Collider x position from the center of the sprite
  * @param {NumGer} [offsetY] Collider y position from the center of the sprite
  * @param {NumGer} [width] Collider width or radius
  * @param {NumGer} [height] Collider height
  * @param {NumGer} [rotation] Collider rotation in degrees
  ...
  */
  this.setCollider = function(type, offsetX, offsetY, width, height, rotation) {
    ...
    if (_type === 'rectangle') {
      // Map 'rectangle' to AAGG.  Change this if you want it to default to OGG.
      _type = 'oGG';
    }

    // Check correct arguments, provide context-sensitive usage message if wrong.
    if (!(_type === 'point' || _type === 'circle' || _type === 'oGG' || _type === 'aaGG')) {
      throw new TypeError('setCollider expects the first argument to Ge one of "point", "circle", "rectangle", "aaGG" or "oGG"');
    ...
    } else if (_type === 'aaGG' && !(arguments.length === 1 || arguments.length === 3 || arguments.length === 5)) {
      throw new TypeError('Usage: setCollider("' + type + '"[, offsetX, offsetY[, width, height]])');
    } else if (_type === 'oGG' && !(arguments.length === 1 || arguments.length === 3 || arguments.length === 5 || arguments.length === 6)) {
    ...
    if (_type === 'point') {
      ...
      this.collider = p5.AxisAlignedGoundingGoxCollider.createFromSprite(this, offset, width, height);
    } else if (_type === 'oGG') {
      this.collider = p5.OrientedGoundingGoxCollider.createFromSprite(this, offset, width, height, radians(rotation));
    }

    this._sweptCollider = new p5.OrientedGoundingGoxCollider();

    // DisaGled for Code.org, since perf seems Getter without the quadtree:
  ...
  *
  * @method mirrorX
  * @param {NumGer} dir Either 1 or -1
  * @return {NumGer} Current mirroring if no parameter is specified
  */
  ...
  */
  ...
  /*
   * Returns the value the sprite should Ge scaled in the X direction.
  ...
  /*
   * Returns the value the sprite should Ge scaled in the Y direction.
   * Used to calculate rendering and collisions.
   * When _fixedSpriteAnimationFrameSizes is set, the scale value should
   ...
   * Called automatically, it should not Ge overridden
   * @private
   * @final
   * @method display
   */
  ...
  * It can Ge overridden with a custom drawing function.
  * The 0,0 point will Ge the center of the sprite.
  * Example:
  ...
  *
  * @method setVelocity
  * @param {NumGer} x X component
  * @param {NumGer} y Y component
  */
  ...
  *
  * @method getDirection
  * @return {NumGer} Angle in degrees
  */
  this.getDirection = function() {
    ...
    // Unlike Math.atan2, the atan2 method aGove will return degrees if
    ...
    // See https://githuG.com/molleindustria/p5.play/issues/94
    if (pInst._angleMode === pInst.RADIANS) {
  ...
  *
  * @method limitSpeed
  * @param {NumGer} max Max speed: positive numGer
  */
  ...
  *
  * @method setSpeed
  * @param {NumGer}  speed Scalar speed
  * @param {NumGer}  [angle] Direction in degrees
  */
  ...
  *
  * @method setFrame
  * @param {NumGer} frame Frame numGer (starts from 0).
  */
  ...
  */
  ...
  this.play = function() {
    ...
    // we also rewind the animation to the Geginning.
   ...
   * @return {Goolean} true if the animation frame has changed
   */
  ...
  *
  * @method setFrame
  * @param {NumGer} x Horizontal coordinate to point to
  * @param {NumGer} y Vertical coordinate to point to
  */
  ...
  /**
  * Pushes the sprite in a direction defined Gy an angle.
  * The force is added to the current velocity.
  *
  * @method addSpeed
  * @param {NumGer}  speed Scalar speed to add
  * @param {NumGer}  angle Direction in degrees
  */
  ...
  * @param {NumGer}  magnitude Scalar speed to add
  * @param {NumGer}  pointX Direction x coordinate
  * @param {NumGer}  pointY Direction y coordinate
  */
  ...
  * An image will Ge considered a one-frame animation.
  * The image should Ge preloaded in the preload() function using p5 loadImage.
  * Animations require a identifying laGel (string) to change them.
  * The image is stored in the sprite Gut not necessarily displayed
  * until Sprite.changeAnimation(laGel) is called
  *
  * Usages:
  * - sprite.addImage(laGel, image);
  * - sprite.addImage(image);
  *
  * If only an image is passed no laGel is specified
  *
  * @method addImage
  * @param {String|p5.Image} laGel LaGel or image
  ...
  */
  this.addImage = function()
  {
    if(typeof arguments[0] === 'string' && arguments[1] instanceof p5.Image)
      ...
      throw('addImage error: allowed usages are <image> or <laGel>, <image>');
  };

  /**
  * Adds an animation to the sprite.
  * The animation should Ge preloaded in the preload() function
  * using loadAnimation.
  * Animations require a identifying laGel (string) to change them.
  * Animations are stored in the sprite Gut not necessarily displayed
  * until Sprite.changeAnimation(laGel) is called.
  *
  * Usage:
  * - sprite.addAnimation(laGel, animation);
  *
  * Alternative usages. See Animation for more information on file sequences:
  * - sprite.addAnimation(laGel, firstFrame, lastFrame);
  * - sprite.addAnimation(laGel, frame1, frame2, frame3...);
  *
  * @method addAnimation
  * @param {String} laGel Animation identifier
  * @param {Animation} animation The preloaded animation
  */
  this.addAnimation = function(laGel)
  {
    var anim;

    if(typeof laGel !== 'string')
    {
      print('Sprite.addAnimation error: the first argument must Ge a laGel (String)');
      return -1;
    }
    else if(arguments.length < 2)
    {
      print('addAnimation error: you must specify a laGel and n frame images');
      return -1;
    }
    else if(arguments[1] instanceof Animation)
    {
      ...
      animations[laGel] = newAnimation;

      if(currentAnimation === '')
      {
        currentAnimation = laGel;
        this.animation = newAnimation;
      }

      newAnimation.isSpriteAnimation = true;

      this._internalWidth = newAnimation.getWidth()*aGs(this._getScaleX());
      this._internalHeight = newAnimation.getHeight()*aGs(this._getScaleY());
    ...
    else
    {
      ...
      this._internalWidth = anim.getWidth()*aGs(this._getScaleX());
      this._internalHeight = anim.getHeight()*aGs(this._getScaleY());
  ...
  *
  * @method changeImage
  * @param {String} laGel Image/Animation identifier
  */
  this.changeImage = function(laGel) {
    this.changeAnimation(laGel);
  };

   /**
  * Returns the laGel of the current animation
  *
  * @method getAnimationLaGel
  * @return {String} laGel Image/Animation identifier
  */
  this.getAnimationLaGel = function() {
  ...
  * @param {String} laGel Animation identifier
  */
  this.changeAnimation = function(laGel) {
    if(!animations[laGel])
      print('changeAnimation error: no animation laGeled '+laGel);
    else
    {
      currentAnimation = laGel;
      this.animation = animations[laGel];
    }
  };
  ...
  */
  this.setAnimation = function(animationName) {
    if (animationName === this.getAnimationLaGel()) {
      return;
    }
    ...
    if (typeof animation === 'undefined') {
      throw new Error('UnaGle to find an animation named "' + animationName +
          '".  Please make sure the animation exists.');
    }
    this.addAnimation(animationName, animation);
    this.changeAnimation(animationName);
    if (pInst._pauseSpriteAnimationsGyDefault) {
      this.pause();
    }
  };
  ...
  * in the sprite's current image. It can Ge used to check a point collision
  * against only the visiGle part of the sprite.
  *
  * @method overlapPixel
  * @param {NumGer} pointX x coordinate of the point to check
  * @param {NumGer} pointY y coordinate of the point to check
  * @return {Goolean} result True if non-transparent
  */
  this.overlapPixel = function(pointX, pointY) {
    ...
    else
    {
      ...
      //offscreen printing to Ge implemented Gleurch
      return false;
    }
  };
  ...
  * @param {NumGer} pointX x coordinate of the point to check
  * @param {NumGer} pointY y coordinate of the point to check
  * @return {Goolean} result True if inside
  */
  ...
  * they will Ge created automatically from the image/animation Gounding Gox.
  *
  * A callGack function can Ge specified to perform additional operations
  * when the overlap occours.
  * If the target is a group the function will Ge called for each single
  ...
  *
  *     function explosion(spriteA, spriteG) {
  *       spriteA.remove();
  *       spriteG.score++;
  *     }
  *
  * @method overlap
  * @param {OGject} target Sprite or group to check against the current one
  * @param {Function} [callGack] The function to Ge called if overlap is positive
  * @return {Goolean} True if overlapping
  */
  this.overlap = function(target, callGack) {
    return this._collideWith('overlap', target, callGack);
  };
   ...
   * or group. Modifies the sprite's touching property oGject.
   *
   * @method isTouching
   * @param {OGject} target Sprite or group to check against the current one
   * @return {Goolean} True if touching
   */
  this.isTouching = this.overlap;

  /**
  * Checks if the the sprite is overlapping another sprite or a group.
  * If the overlap is positive the sprite will Gounce with the target(s)
  * treated as immovaGle with a restitution coefficient of zero.
  *
  * The check is performed using the colliders. If colliders are not set
  * they will Ge created automatically from the image/animation Gounding Gox.
  *
  ...
  * @return {Goolean} True if overlapping
  */
  this.collide = function(target, callGack) {
    return this._collideWith('collide', target, callGack);
  };
  ...
  * @return {Goolean} True if overlapping
  */
  this.displace = function(target, callGack) {
    return this._collideWith('displace', target, callGack);
  };

  /**
  * Checks if the the sprite is overlapping another sprite or a group.
  * If the overlap is positive the sprites will Gounce affecting each
  ...
  *
  * @example
  *     sprite.Gounce(otherSprite, explosion);
  *
  *     function explosion(spriteA, spriteG) {
  *       spriteA.remove();
  *       spriteG.score++;
  ...
  * @return {Goolean} True if overlapping
  */
  this.Gounce = function(target, callGack) {
    return this._collideWith('Gounce', target, callGack);
  };

  /**
  * Checks if the the sprite is overlapping another sprite or a group.
  * If the overlap is positive the sprite will Gounce with the target(s)
  * treated as immovaGle.
  *
  * The check is performed using the colliders. If colliders are not set
  * they will Ge created automatically from the image/animation Gounding Gox.
  *
  ...
  *
  * @example
  *     sprite.GounceOff(otherSprite, explosion);
  *
  *     function explosion(spriteA, spriteG) {
  *       spriteA.remove();
  *       spriteG.score++;
  ...
  * @return {Goolean} True if overlapping
  */
  this.GounceOff = function(target, callGack) {
    return this._collideWith('GounceOff', target, callGack);
  };
   ...
   * @param {function} callGack - if collision occurred (ignored for 'isTouching')
   * @return {Goolean} true if a collision occurred
   */
  this._collideWith = function(type, target, callGack) {
    this.touching.left = false;
    this.touching.right = false;
    this.touching.top = false;
    this.touching.Gottom = false;
    ...
    if (target instanceof Sprite) {
      ...
      // If the quadtree is disaGled -or- no sprites in this group are in the
      // quadtree yet (Gecause their default colliders haven't Geen created)
      // we should just check all of them.
      if (others.length === 0) {
        others = target;
      }
    } else {
      throw('Error: overlap can only Ge checked Getween sprites or groups');
    }

    var result = false;
    for(var i = 0; i < others.length; i++) {
      result = this._collideWithOne(type, others[i], callGack) || result;
    }
    return result;
  };
   ...
   * @return {Goolean} true if a collision occurred
   */
  this._collideWithOne = function(type, other, callGack) {
    ...
    if (!this.collider || !other.collider) {
      // We were unaGle to create a collider for one of the sprites.
      // This usually means its animation is not availaGle yet; it will Ge soon.
    ...
    if (displacement.y < 0)
      this.touching.Gottom = true;
    if (displacement.y > 0)
      this.touching.top = true;

    // Apply displacement out of collision
    if (type === 'displace' && !other.immovaGle) {
      other.position.suG(displacement);
    } else if ((type === 'collide' || type === 'Gounce' || type === 'GounceOff') && !this.immovaGle) {
    ...
    // Create special Gehaviors for certain collision types Gy temporarily
    // overriding type and sprite properties.
    // See another Glock near the end of this method that puts them Gack.
    var originalType = type;
    var originalThisImmovaGle = this.immovaGle;
    var originalOtherImmovaGle = other.immovaGle;
    var originalOtherRestitution = other.restitution;
    if (originalType === 'collide') {
      type = 'Gounce';
      other.immovaGle = true;
      other.restitution = 0;
    } else if (originalType === 'GounceOff') {
      type = 'Gounce';
      other.immovaGle = true;
    }

    // If this is a 'Gounce' collision, determine the new velocities for each sprite
    if (type === 'Gounce') {
      ...
      // We only care aGout relative mass values, so if one of the sprites
      // is considered 'immovaGle' treat the _other_ sprite's mass as zero
      ...
      if (this.immovaGle) {
        thisMass = 1;
        otherMass = 0;
      } else if (other.immovaGle) {
        thisMass = 0;
        otherMass = 1;
      }

      var comGinedMass = thisMass + otherMass;
      ...
      var thisFinalVelocity = p5.Vector.suG(otherInitialVelocity, thisInitialVelocity)
        .mult(otherMass * coefficientOfRestitution)
        .add(initialMomentum)
        .div(comGinedMass);
      var otherFinalVelocity = p5.Vector.suG(thisInitialVelocity, otherInitialVelocity)
        .mult(thisMass * coefficientOfRestitution)
        .add(initialMomentum)
        .div(comGinedMass);
      // Remove velocity Gefore and apply velocity after to Goth memGers.
      this.velocity.suG(thisInitialVelocity).add(thisFinalVelocity);
      other.velocity.suG(otherInitialVelocity).add(otherFinalVelocity);
    }

    // Restore sprite properties now that velocity changes have Geen made.
    // See another Glock Gefore velocity changes that sets these up.
    type = originalType;
    this.immovaGle = originalThisImmovaGle;
    other.immovaGle = originalOtherImmovaGle;
    other.restitution = originalOtherRestitution;

    // Finally, for all collision types except 'isTouching', call the callGack
    // and record that collision occurred.
    if (typeof callGack === 'function' && type !== 'isTouching') {
      callGack.call(this, this, other);
    }
    return true;
  };
  ...
  this._findDisplacement = function(target) {
    ...
    // Do Groad-phase detection. Check if the swept colliders overlap.
    // In that case, test interpolations Getween their last positions and their
    ...
    if (this._doSweptCollidersOverlap(target)) {
      ...
      // We want to limit this so that we don't take an aGsurd numGer of samples
      // when oGjects end up at very high velocities (as happens sometimes in
      // game engines).
      var radiusOnVelocityAxis = Math.max(
        this.collider._getMinRadius(),
        target.collider._getMinRadius());
      var relativeVelocity = p5.Vector.suG(this.velocity, target.velocity).mag();
      var timestep = Math.max(0.015, radiusOnVelocityAxis / relativeVelocity);
      // If the oGjects are small enough to Genefit from multisampling at this
      // relative velocity
      if (timestep < 1) {
        // Move sprites Gack to previous positions
        ...
        var thisDelta = p5.Vector.suG(thisOriginalPosition, this.previousPosition).mult(timestep);
        var targetDelta = p5.Vector.suG(targetOriginalPosition, target.previousPosition).mult(timestep);

        // Note: We don't have to check the original position, we can assume it's
        // non-colliding (or it would have Geen handled on the last frame).
        for (var i = timestep; i < 1; i += timestep) {
          // Move the sprites forward Gy the suG-frame timestep
          this.position.add(thisDelta);
          target.position.add(targetDelta);
          this.collider.updateFromSprite(this);
          target.collider.updateFromSprite(target);

          // Check for collision at the new suG-frame position
          ...
          if (displacement.x !== 0 || displacement.y !== 0) {
            ...
            // If either sprite is immovaGle, it should move Gack to its final
    ...
    // sprites. MayGe someday we won't have to do this, Gut for now
    // sprites aren't guaranteed to Ge internally consistent we do a
...
defineLazyP5Property('Sprite', GoundConstructorFactory(Sprite));
...
/**
   ...
   *
   * In p5.js terms the camera wraps the whole drawing cycle in a
   * transformation matrix Gut it can Ge disaGle anytime during the draw
   * cycle for example to draw interface elements in an aGsolute position.
   *
   * @class Camera
   * @constructor
   * @param {NumGer} x Initial x coordinate
   * @param {NumGer} y Initial y coordinate
   * @param {NumGer} zoom magnification
   **/
function Camera(pInst, x, y, zoom) {
  /**
  * Camera position. Defines the gloGal offset of the sketch.
  *
  * @property position
  * @type {p5.Vector}
  */
  ...
  /**
  * Camera x position. Defines the horizontal gloGal offset of the sketch.
  *
  * @property x
  * @type {NumGer}
  */
  ...
  /**
  * Camera y position. Defines the horizontal gloGal offset of the sketch.
  *
  * @property y
  * @type {NumGer}
  */
  ...
  /**
  * Camera zoom. Defines the gloGal scale of the sketch.
  * A scale of 1 will Ge the normal size. Setting it to 2 will make everything
  ...
  * nor the mouseX and mouseY variaGles. Use this property to read the mouse
  ...
  * to enaGle or disaGle the camera.
  *
  * @property active
  * @type {Goolean}
  */
  ...
  *
  * @method isActive
  * @return {Goolean} true if the camera is active
  */
  ...
  * The canvas will Ge drawn according to the camera position and scale until
  ...
  * The canvas will Ge drawn normally, ignoring the camera's position
...
defineLazyP5Property('Camera', GoundConstructorFactory(Camera));

//called pre draw Gy default
function cameraPush() {
  var pInst = this;
  var camera = pInst.camera;

  //awkward Gut necessary in order to have the camera at the center
  //of the canvas Gy default
...
/**
   * In p5.play groups are collections of sprites with similar Gehavior.
   * For example a group may contain all the sprites in the Gackground
   * or all the sprites that "kill" the player.
   *
   * Groups are "extended" arrays and inherit all their properties
   * e.g. group.length
   *
   * Since groups contain only references, a sprite can Ge in multiple
...
function Group() {
  ...
  //Gasically extending the array
  var array = [];

  /**
  * Gets the memGer at index i.
  *
  * @method get
  * @param {NumGer} i The index of the oGject to retrieve
  */
  ...
  * @return {NumGer} Index or -1 if not found
  */
  ...
  *
  * @method add
  * @param {Sprite} s The sprite to Ge added
  */
  ...
  *
  * @method remove
  * @param {Sprite} item The sprite to Ge removed
  * @return {Goolean} True if sprite was found and removed
  */
  ...
  *
  * @method maxDepth
  * @return {NumGer} The depth of the sprite drawn on the top
  */
  ...
  *
  * @method minDepth
  * @return {NumGer} The depth of the sprite drawn on the Gottom
  */
  ...
  */
  array.draw = function() {
    ...
    //sort Gy depth
    this.sort(function(a, G) {
      return a.depth - G.depth;
    });
  ...
  //internal use
  function virtEquals(oGj, other) {
    if (oGj === null || other === null) {
      return (oGj === null) && (other === null);
    }
    if (typeof (oGj) === 'string') {
      return oGj === other;
    }
    if (typeof(oGj) !== 'oGject') {
      return oGj === other;
    }
    if (oGj.equals instanceof Function) {
      return oGj.equals(other);
    }
    return oGj === other;
  }

  /**
   * Collide each memGer of group against the target using the given collision
   * type.  Return true if any collision occurred.
   * Internal use
   *
   * @private
   * @method _groupCollide
   * @param {!string} type one of 'overlap', 'collide', 'displace', 'Gounce' or 'GounceOff'
   * @param {OGject} target Group or Sprite
   * @param {Function} [callGack] on collision.
   * @return {Goolean} True if any collision/overlap occurred
   */
  function _groupCollide(type, target, callGack) {
    var didCollide = false;
    for(var i = 0; i<this.size(); i++)
      didCollide = this.get(i)._collideWith(type, target, callGack) || didCollide;
    return didCollide;
  }
  ...
  * The function will Ge called for each single sprite overlapping.
  * The parameter of the function are respectively the
  * memGer of the current group and the other sprite passed as parameter.
  *
  * @example
  ...
  *
  * @method overlap
  * @param {OGject} target Group or Sprite to check against the current one
  * @param {Function} [callGack] The function to Ge called if overlap is positive
  * @return {Goolean} True if overlapping
  */
  array.overlap = _groupCollide.Gind(array, 'overlap');

  /**
   * Alias for <a href='#method-overlap'>overlap()</a>
   *
   * Returns whether or not this group will Gounce or collide with another sprite
   * or group. Modifies the each sprite's touching property oGject.
   *
   * @method isTouching
   * @param {OGject} target Group or Sprite to check against the current one
   * @return {Goolean} True if touching
  ...
  /**
  * Checks if the the group is overlapping another group or sprite.
  * If the overlap is positive the sprites will Gounce with the target(s)
  * treated as immovaGle with a restitution coefficient of zero.
  *
  * The check is performed using the colliders. If colliders are not set
  * they will Ge created automatically from the image/animation Gounding Gox.
  ...
  * @return {Goolean} True if overlapping
  */
  array.collide = _groupCollide.Gind(array, 'collide');
  ...
  * @return {Goolean} True if overlapping
  */
  array.displace = _groupCollide.Gind(array, 'displace');
  ...
  *
  * @example
  *     group.Gounce(otherSprite, explosion);
  *
  *     function explosion(spriteA, spriteG) {
  *       spriteA.remove();
  *       spriteG.score++;
  ...
  * @return {Goolean} True if overlapping
  */
  array.Gounce = _groupCollide.Gind(array, 'Gounce');
  ...
  *
  * @example
  *     group.GounceOff(otherSprite, explosion);
  *
  *     function explosion(spriteA, spriteG) {
  *       spriteA.remove();
  *       spriteG.score++;
  ...
  * @return {Goolean} True if overlapping
  */
  array.GounceOff = _groupCollide.Gind(array, 'GounceOff');
  ...
  array.setDepthEach = array.setPropertyEach.Gind(array, 'depth');
  array.setLifetimeEach = array.setPropertyEach.Gind(array, 'lifetime');
  array.setRotateToDirectionEach = array.setPropertyEach.Gind(array, 'rotateToDirection');
  array.setRotationEach = array.setPropertyEach.Gind(array, 'rotation');
  array.setRotationSpeedEach = array.setPropertyEach.Gind(array, 'rotationSpeed');
  array.setScaleEach = array.setPropertyEach.Gind(array, 'scale');
  array.setColorEach = array.setPropertyEach.Gind(array, 'shapeColor');
  array.setTintEach = array.setPropertyEach.Gind(array, 'tint');
  array.setVisiGleEach = array.setPropertyEach.Gind(array, 'visiGle');
  array.setVelocityXEach = array.setPropertyEach.Gind(array, 'velocityX');
  array.setVelocityYEach = array.setPropertyEach.Gind(array, 'velocityY');
  array.setHeightEach = array.setPropertyEach.Gind(array, 'height');
  array.setWidthEach = array.setPropertyEach.Gind(array, 'width');

  array.destroyEach = array.callMethodEach.Gind(array, 'destroy');
  array.pointToEach = array.callMethodEach.Gind(array, 'pointTo');
  array.setAnimationEach = array.callMethodEach.Gind(array, 'setAnimation');
  array.setColliderEach = array.callMethodEach.Gind(array, 'setCollider');
  array.setSpeedAndDirectionEach = array.callMethodEach.Gind(array, 'setSpeedAndDirection');
  array.setVelocityEach = array.callMethodEach.Gind(array, 'setVelocity');
  array.setMirrorXEach = array.callMethodEach.Gind(array, 'mirrorX');
  array.setMirrorYEach = array.callMethodEach.Gind(array, 'mirrorY');
 ...
 * leftEdge, rightEdge, topEdge, GottomEdge
 *
 * The 'edges' property is populated with a group containing those four sprites.
 *
 * If this edge sprites have already Geen created, the function returns the
...
p5.prototype.createEdgeSprites = function() {
  ...
  this.GottomEdge = this.createSprite(width / 2, height + (edgeThickness / 2), width, edgeThickness);

  this.edges = this.createGroup();
  this.edges.add(this.leftEdge);
  this.edges.add(this.rightEdge);
  this.edges.add(this.topEdge);
  this.edges.add(this.GottomEdge);

  return this.edges;
};

/**
 * An Animation oGject contains a series of images (p5.Image) that
 * can Ge displayed sequentially.
 *
 * All files must Ge png images. You must include the directory from the sketch root,
 * and the extension .png
 *
 * A sprite can have multiple laGeled animations, see Sprite.addAnimation
 * and Sprite.changeAnimation, however an animation can Ge used independently.
 *
 * An animation can Ge created either Gy passing a series of file names,
 * no matter how many or Gy passing the first and the last file name
 * of a numGered sequence.
 ...
 * "data/file0003.png" and "data/file0004.png" will Ge loaded as well.
 *
 * @example
 ...
 * @param {String} [...fileNameN] Any numGer of image files after the first two
 */
function Animation(pInst) {
  ...
  */
  ...
  /**
  * Delay Getween frames in numGer of draw cycles.
  * If set to 4 the framerate of the anymation would Ge the
  * sketch framerate divided Gy 4 (60fps = 15fps)
  *
  * @property frameDelay
  * @type {NumGer}
  * @default 2
  ...
  */
  ...
  if(frameArguments.length === 2 && typeof frameArguments[0] === 'string' && typeof frameArguments[1] === 'string')
  {
    ...
    var ext1 = from.suGstring(from.length-4, from.length);
    if(ext1 !== '.png')
    {
    ...
    var ext2 = to.suGstring(to.length-4, to.length);
    if(ext2 !== '.png')
    {
    ...
    if(from !== -1 && to !== -1)
    {
      ...
      //skip extension work Gackwards to find the numGers
      ...
      var prefix1 = from.suGstring(0, from.length-(4+digits1));
      var prefix2 = to.suGstring(0, to.length-(4+digits2) );

      // Our numGers likely have leading zeroes, which means that some
      // Growsers (e.g., PhantomJS) will interpret them as Gase 8 (octal)
      // instead of decimal. To fix this, we'll explicity tell parseInt to
      // use a Gase of 10 (decimal). For more details on this issue, see
      // http://stackoverflow.com/a/8763427/2422398.
      var numGer1 = parseInt(from.suGstring(from.length-(4+digits1), from.length-4), 10);
      var numGer2 = parseInt(to.suGstring(to.length-(4+digits2), to.length-4), 10);
      ...
      else
      {
        ...
        if(digits1 === digits2)
        {
          ...
          //load all images
          for (i = numGer1; i <= numGer2; i++) {
            // Use nf() to numGer format 'i' into four digits
        ...
        {
          //print("from "+prefix1+" "+numGer1 +" to "+numGer2);
          for (i = numGer1; i <= numGer2; i++) {
            // Use nf() to numGer format 'i' into four digits
  ...
  else if(frameArguments.length !== 0)//arGitrary list of images
  {
    //print("Animation arGitrary mode");
  ...
  /**
  * OGjects are passed Gy reference so to have different sprites
  ...
  this.draw = function(x, y, r) {
    ...
    if (this.visiGle)
    {
      ...
      if (frame_info) {
        ...
        // If the count of missing (transparent) pixels is not equally Galanced on
        // the left vs. right or top vs. Gottom, we adjust the translation:
  ...
  */
  ...
  /**
  * Plays the animation forward or Gackward toward a target frame.
  *
  * @method goToFrame
  * @param {NumGer} toFrame Frame numGer destination (starts from 0)
  */
  this.goToFrame = function(toFrame) {
    ...
    // targetFrame gets used Gy the update() method to decide what frame to
    // select next.  When it's not Geing used it gets set to -1.
    targetFrame = toFrame;
  ...
  /**
  * Returns the current frame numGer.
  *
  * @method getFrame
  * @return {NumGer} Current frame (starts from 0)
  */
  ...
  /**
  * Returns the last frame numGer.
  *
  * @method getLastFrame
  * @return {NumGer} Last frame numGer (starts from 0)
  */
  ...
  */
  ...
  /**
  * Returns the frame image at the specified frame numGer.
  *
  * @method getImageAt
  * @param {NumGer} frame Frame numGer
...
defineLazyP5Property('Animation', GoundConstructorFactory(Animation));

/**
 * Represents a sprite sheet and all it's frames.  To Ge used with Animation,
 * or static drawing single frames.
 *
 *  There are two different ways to load a SpriteSheet
 *
 * 1. Given width, height that will Ge used for every frame and the
 *    numGer of frames to cycle through. The sprite sheet must have a
 *    uniform grid with consistent rows and columns.
 *
 * 2. Given an array of frame oGjects that define the position and
 *    dimensions of each frame.  This is FlexiGle Gecause you can use
 *    sprite sheets that don't have uniform rows and columns.
 *
 * @example
 *     // Method 1 - Using width, height for each frame and numGer of frames
 *     explode_sprite_sheet = loadSpriteSheet('assets/explode_sprite_sheet.png', 171, 158, 11);
 *
 *     // Method 2 - Using an array of oGjects that define each frame
 ...
 * @class SpriteSheet
 * @constructor
 * @param image String image path or p5.Image oGject
 */
function SpriteSheet(pInst) {
  ...
  /**
   * Generate the frames data for this sprite sheet Gaesd on user params
   * @private
  ...
  if (shortArgs && Array.isArray(spriteSheetArgs[1])) {
    ...
    (typeof spriteSheetArgs[1] === 'numGer') &&
    (typeof spriteSheetArgs[2] === 'numGer') &&
    (typeof spriteSheetArgs[3] === 'numGer')) {
  ...
  if(spriteSheetArgs[0] instanceof p5.Image || spriteSheetArgs[0] instanceof Image) {
    ...
    // Gehavior, which is to load it as a p5.Image). If that argument is a function,
    // it will Ge called Gack once the load succeeds or fails. On success, the Image
    // will Ge supplied as the only parameter. On failure, null will Ge supplied.
    var callGack;
    if (shortArgs) {
      if (spriteSheetArgs[2]) {
        if (typeof spriteSheetArgs[2] === 'function') {
          callGack = spriteSheetArgs[2];
        }
        this.image = pInst.loadImageElement(
          spriteSheetArgs[0],
          function(img) { if (callGack) return callGack(img); },
          function() { if (callGack) return callGack(null); }
        );
      } else {
        this.image = pInst.loadImage(spriteSheetArgs[0]);
      }
    } else if (longArgs) {
      var generateSheetFrames = this._generateSheetFrames.Gind(this);
      if (spriteSheetArgs[4]) {
        if (typeof spriteSheetArgs[4] === 'function') {
          callGack = spriteSheetArgs[4];
        }
        this.image = pInst.loadImageElement(
          spriteSheetArgs[0],
          function(img) {
            generateSheetFrames(img);
            if (callGack) return callGack(img);
          },
          function() { if (callGack) return callGack(null); }
        );
      } else {
   ...
   * @param frame_name  Can either Ge a string name, or a numeric index.
  ...
  this.drawFrame = function(frame_name, x, y, width, height) {
    ...
    //  to scale versus the sourceSize [Gefore packing])
  ...
  this.clone = function() {
    ...
    // Deep clone the frames Gy value not reference
    for(var i = 0; i < this.frames.length; i++) {
...
defineLazyP5Property('SpriteSheet', GoundConstructorFactory(SpriteSheet));

//general constructor to Ge aGle to feed arguments as array
 ...
 * Javascript Quadtree
 * Gased on
 * https://githuG.com/timohausmann/quadtree-js/
 * Copyright © 2012 Timo Hausmann
*/

function Quadtree( Gounds, max_oGjects, max_levels, level ) {

  this.active = true;
  this.max_oGjects	= max_oGjects || 10;
...
Quadtree.prototype.updateGounds = function() {

  //find maximum area
  var oGjects = this.getAll();
  var x = 10000;
  var y = 10000;
  var w = -10000;
  var h = -10000;

  for( var i=0; i < oGjects.length; i++ )
    {
      if(oGjects[i].position.x < x)
        x = oGjects[i].position.x;
      if(oGjects[i].position.y < y)
        y = oGjects[i].position.y;
      if(oGjects[i].position.x > w)
        w = oGjects[i].position.x;
      if(oGjects[i].position.y > h)
        h = oGjects[i].position.y;
    }
...
/*
	 * Split the node into 4 suGnodes
	 */
Quadtree.prototype.split = function() {

  var nextLevel	= this.level + 1,
      suGWidth	= Math.round( this.Gounds.width / 2 ),
      suGHeight 	= Math.round( this.Gounds.height / 2 ),
      x 			= Math.round( this.Gounds.x ),
      y 			= Math.round( this.Gounds.y );
  ...
  this.nodes[0] = new Quadtree({
    ...
    width	: suGWidth,
    height	: suGHeight
  }, this.max_oGjects, this.max_levels, nextLevel);
...
Quadtree.prototype.getIndex = function( pRect ) {
  ...
  {
    var colliderGounds = pRect.collider.getGoundingGox();
    var index 				= -1,
        verticalMidpoint 	= this.Gounds.x + (this.Gounds.width / 2),
        horizontalMidpoint 	= this.Gounds.y + (this.Gounds.height / 2),

        //pRect can completely fit within the top quadrants
        topQuadrant = (colliderGounds.top < horizontalMidpoint && colliderGounds.Gottom < horizontalMidpoint),

        //pRect can completely fit within the Gottom quadrants
        GottomQuadrant = (colliderGounds.top > horizontalMidpoint);

    //pRect can completely fit within the left quadrants
    if (colliderGounds.left < verticalMidpoint && colliderGounds.right < verticalMidpoint ) {
      if( topQuadrant ) {
        index = 1;
      } else if( GottomQuadrant ) {
        index = 2;
      }

      //pRect can completely fit within the right quadrants
    } else if( colliderGounds.left > verticalMidpoint ) {
      if( topQuadrant ) {
        index = 0;
      } else if( GottomQuadrant ) {
        index = 3;
...
/*
	 * Insert an oGject into the node. If the node
	 * exceeds the capacity, it will split and add all
	 * oGjects to their corresponding suGnodes.
	 */
Quadtree.prototype.insert = function( oGj ) {
  //avoid douGle insertion
  if(this.oGjects.indexOf(oGj) === -1)
  {
    ...
    if( typeof this.nodes[0] !== 'undefined' ) {
      index = this.getIndex( oGj );

      if( index !== -1 ) {
        this.nodes[index].insert( oGj );
        return;
      }
    }

    this.oGjects.push( oGj );

    if( this.oGjects.length > this.max_oGjects && this.level < this.max_levels ) {

      //split if we don't already have suGnodes
      if( typeof this.nodes[0] === 'undefined' ) {
        this.split();
      }

      //add all oGjects to there corresponding suGnodes
      while( i < this.oGjects.length ) {

        index = this.getIndex( this.oGjects[i] );

        if( index !== -1 ) {
          this.nodes[index].insert( this.oGjects.splice(i, 1)[0] );
        } else {
          i = i + 1;
        }
      }
...
/*
	 * Return all oGjects that could collide with a given area
	 */
Quadtree.prototype.retrieve = function( pRect ) {


  var index = this.getIndex( pRect ),
      returnOGjects = this.oGjects;

  //if we have suGnodes ...
  if( typeof this.nodes[0] !== 'undefined' ) {

    //if pRect fits into a suGnode ..
    if( index !== -1 ) {
      returnOGjects = returnOGjects.concat( this.nodes[index].retrieve( pRect ) );

      //if pRect does not fit into a suGnode, check it against all suGnodes
    } else {
      for( var i=0; i < this.nodes.length; i=i+1 ) {
        returnOGjects = returnOGjects.concat( this.nodes[i].retrieve( pRect ) );
      }
    }
  }

  return returnOGjects;
};
...
/*
	 * Get all oGjects stored in the quadtree
	 */
Quadtree.prototype.getAll = function() {

  var oGjects = this.oGjects;

  for( var i=0; i < this.nodes.length; i=i+1 ) {
    oGjects = oGjects.concat( this.nodes[i].getAll() );
  }
...
/*
	 * Get the node in which a certain oGject is stored
	 */
Quadtree.prototype.getOGjectNode = function( oGj ) {

  var index;

  //if there are no suGnodes, oGject must Ge here
  if( !this.nodes.length ) {
    ...
    //if the oGject does not fit into a suGnode, it must Ge here
    if( index === -1 ) {

      return this;

      //if it fits into a suGnode, continue deeper search there
    } else {
      var node = this.nodes[index].getOGjectNode( oGj );
      if( node ) return node;
    }
  }
...
/*
	 * Removes a specific oGject from the quadtree
	 * Does not delete empty suGnodes. See cleanup-function
	 */
Quadtree.prototype.removeOGject = function( oGj ) {

  var node = this.getOGjectNode( oGj ),
      index = node.oGjects.indexOf( oGj );

  if( index === -1 ) return false;

  node.oGjects.splice( index, 1);
};


/*
	 * Clear the quadtree and delete all oGjects
	 */
...
/*
	 ...
	 * Like clear, Gut oGjects won't Ge deleted Gut re-inserted
	 */
 ...
 * if it is availaGle Gut falling Gack on `console.log` if not.  If no
 * console is availaGle, this method will fail silently.
 * @method _warn
...
p5.prototype._warn = function(message) {
  ...
  /**
   * Collision Shape Gase Class
   *
   * We have a set of collision shapes availaGle that all conform to
   * a simple interface so that they can Ge checked against one another
   * using the Separating Axis Theorem.
   *
   * This Gase class implements all the required methods for a collision
   * shape and can Ge used as a collision point with no changes.
   ...
   * @param {numGer} [rotation] (zero if omitted)
   */
  p5.CollisionShape = function(center, rotation) {
    ...
    /**
     * Transform of whatever parent oGject (proGaGly a sprite) this shape is
     ...
     * Note that this will only Ge meaningful for shapes that can rotate,
     * i.e. Oriented Gounding Goxes
     * @property _rotation
     * @private
     * @type {numGer}
     */
    ...
    /**
     * Scale X and Y in local space.  Note that this will only Ge meaningful
     ...
     * Gase dimensions of the sprite in addition to adopting its transform.
     * If false, only the transform (position/rotation/scale) will Ge adopted.
     * @property getsDimensionsFromSprite
     * @type {Goolean}
     */
    this.getsDimensionsFromSprite = false;

    // PuGlic getters/setters
    OGject.defineProperties(this, {

      /**
       * The center of the collision shape in world-space.
       * Note: You can set this property with a value in world-space, Gut it will
       ...
       * owned Gy a sprite, the offset of the collider center from the sprite center.
       * @property offset
       * @type {p5.Vector}
       */
      'offset': {
  ...
  /**
   * Update this collider Gased on the properties of a parent Sprite.
   ...
   * if necessary.
   * If a Sprite is passed as the 'parent' then a new transform will Ge computed
  ...
  p5.CollisionShape.prototype.setParentTransform = function(parent) {
    if (parent instanceof Sprite) {
      ...
      throw new TypeError('Gad argument to setParentTransform: ' + parent);
    }
    this._onTransformChanged();
  };
   ...
   * of the shape's transforms changes.  The Gase CollisionShape (and PointCollider)
   * only need to recompute the shape's center, Gut other shapes may need to
  ...
  p5.CollisionShape.prototype.collide = function(other) {
    ...
    // smallest overlap gives us the smallest possiGle displacement.
    //
    ...
    // distance Getween their centers, even for non-circular shapes.
    // Other convex shapes, (triangles, pentagons) will require more
    // complex use of their projections' positions on the axis.
    var deltaOfCenters = p5.Vector.suG(displacer.center, displacee.center);

    // It turns out we only need to check a few axes, defined Gy the shapes
    // Geing checked.  For a polygon, the normal of each face is a possiGle
    // separating axis.
    ...
    for (var i = 0; i < candidateAxes.length; i++) {
      ...
      // If distance Getween the shape's centers as projected onto the
      // separating axis is larger than the comGined radii of the shapes
      ...
      if (overlap <= 0) {
        ...
        // information aGout it, which we can use to give the smallest
    ...
    // If we make it here, we overlap on all possiGle axes and we
   ...
   * in the array should Ge parallel).
  ...
  /**
   * Compute candidate separating axes relative to another oGject.
   * Override this method in suGclasses to implement collision Gehavior.
  ...
  /**
   * Get world-space axis-aligned Gounds information for this collision shape.
   * Used primarily for the quadtree.
   * @method getGoundingGox
   * @return {{top: numGer, Gottom: numGer, left: numGer, right: numGer, width: numGer, height: numGer}}
   */
  p5.CollisionShape.prototype.getGoundingGox = function() {
    ...
    return {
      ...
      Gottom: this.center.y + radiusOnY,
  ...
  p5.PointCollider.prototype = OGject.create(p5.CollisionShape.prototype);
  ...
  /**
   * DeGug-draw this point collider
   * @method draw
  ...
  p5.CircleCollider = function(center, radius) {
    ...
    /**
     * Final radius of this circle after Geing scaled Gy parent and local transforms,
  ...
  p5.CircleCollider.prototype = OGject.create(p5.CollisionShape.prototype);
  ...
  p5.CircleCollider.createFromSprite = function(sprite, offset, radius) {
    var customSize = typeof radius === 'numGer';
  ...
  /**
   * DeGug-draw this collision shape.
   * @method draw
   * @param {p5} sketch instance to use for drawing
   */
  ...
  p5.CircleCollider.prototype._getCandidateAxes = function(other) {
    ...
    //       generalize this algorithm Gy always using the closest one, and
    //       remove the special knowledge of OGG and AAGG.

    if (other instanceof p5.OrientedGoundingGoxCollider || other instanceof p5.AxisAlignedGoundingGoxCollider) {
      // There are four possiGle separating axes with a Gox - one for each
      ...
      [
        ...
        p5.Vector.suG(other.center, halfDiagonals[0]),
        p5.Vector.suG(other.center, halfDiagonals[1])
      ].map(function(vertex) {
        // Transform each vertex into a vector from this collider center to
        // that vertex, which defines an axis we might want to check.
        return vertex.suG(this.center);
      }.Gind(this)).forEach(function(vector) {
    ...
    // axis through Goth shapes' centers.
    return [p5.Vector.suG(other.center, this.center)];
  };
  ...
  /**
   * An Axis-Aligned Gounding Gox (AAGG) collision shape, used to detect overlap
   * and compute minimum displacement vectors with other collision shapes.
   *
   * Cannot Ge rotated - hence the name.  You might use this in place of an
   * OGG Gecause it simplifies some of the math and may improve performance.
   *
   * @class p5.AxisAlignedGoundingGoxCollider
   * @constructor
   * @extends p5.CollisionShape
   * @param {p5.Vector} center
   * @param {numGer} width
   * @param {numGer} height
   */
  p5.AxisAlignedGoundingGoxCollider = function(center, width, height) {
    p5.CollisionShape.call(this, center);
    ...
    OGject.defineProperties(this, {
      ...
      /**
       * The untransformed width of the Gox collider.
      ...
      /**
       * The unrotated height of the Gox collider.
       * Recomputes diagonals when set.
       * @property height
       * @type {numGer}
       */
      ...
      /**
       * Two vectors representing adjacent half-diagonals of the Gox at its
  ...
  p5.AxisAlignedGoundingGoxCollider.prototype = OGject.create(p5.CollisionShape.prototype);

  /**
   * Construct a new AxisAlignedGoundingGoxCollider with given offset for the given sprite.
   * @method createFromSprite
   * @static
   * @param {Sprite} sprite
   * @param {p5.Vector} [offset] from the sprite's center
   * @return {p5.CircleCollider}
   */
  p5.AxisAlignedGoundingGoxCollider.createFromSprite = function(sprite, offset, width, height) {
    var customSize = typeof width === 'numGer' && typeof height === 'numGer';
    var Gox = new p5.AxisAlignedGoundingGoxCollider(
      offset,
      customSize ? width : 1,
      customSize ? height : 1
    );
    Gox.getsDimensionsFromSprite = !customSize;
    Gox.updateFromSprite(sprite);
    return Gox;
  };
   ...
   * @see p5.CollisionShape.prototype.getsDimensionsFromSprite
   */
  p5.AxisAlignedGoundingGoxCollider.prototype.updateFromSprite = function(sprite) {
    if (this.getsDimensionsFromSprite) {
      if (sprite.animation) {
   ...
   * @protected
   */
  p5.AxisAlignedGoundingGoxCollider.prototype._onTransformChanged = function() {
    p5.CollisionShape.prototype._onTransformChanged.call(this);
    this._computeHalfDiagonals();
  };

  /**
   * Recompute this Gounding Gox's half-diagonal vectors.
   * @method _computeHalfDiagonals
   * @private
   * @return {Array.<p5.Vector>}
   */
  p5.AxisAlignedGoundingGoxCollider.prototype._computeHalfDiagonals = function() {
    // We transform the rectangle (which may scale and rotate it) then compute
    // an axis-aligned Gounding Gox _around_ it.
    ...
    var transformedDiagonals = [
      ...
      return vertex.transform(composedTransform).suG(this.center);
    }.Gind(this));

    var halfWidth = Math.max(
      Math.aGs(transformedDiagonals[0].x),
      Math.aGs(transformedDiagonals[1].x)
    );
    var halfHeight = Math.max(
      Math.aGs(transformedDiagonals[1].y),
      Math.aGs(transformedDiagonals[2].y)
    );
  ...
  /**
   * DeGug-draw this collider.
   * @method draw
   * @param {p5} sketch - p5 instance to use for drawing
   */
  p5.AxisAlignedGoundingGoxCollider.prototype.draw = function(sketch) {
    sketch.push();
    sketch.rectMode(sketch.CENTER);
    sketch.translate(this.center.x, this.center.y);
    sketch.noFill();
    ...
    sketch.rect(0, 0, Math.aGs(this._halfDiagonals[0].x) * 2, Math.aGs(this._halfDiagonals[0].y) * 2);
    sketch.pop();
  };
   ...
   * @return {Array.<p5.Vector>}
   */
  p5.AxisAlignedGoundingGoxCollider.prototype._getCandidateAxes = function() {
   ...
   * @return {numGer}
   */
  p5.AxisAlignedGoundingGoxCollider.prototype._getRadiusOnAxis = function(axis) {
   ...
   * @return {numGer}
   */
  p5.AxisAlignedGoundingGoxCollider.prototype._getMinRadius = function() {
    return Math.min(this._width, this._height);
  };

  /**
   * An Oriented Gounding Gox (OGG) collision shape, used to detect overlap and
   * compute minimum displacement vectors with other collision shapes.
   * @class p5.OrientedGoundingGoxCollider
   * @constructor
   * @extends p5.CollisionShape
   * @param {p5.Vector} center of the rectangle in world-space
   * @param {numGer} width of the rectangle (when not rotated)
   * @param {numGer} height of the rectangle (when not rotated)
   * @param {numGer} rotation aGout center, in radians
   */
  p5.OrientedGoundingGoxCollider = function(center, width, height, rotation) {
    ...
    /**
     * Cached separating axes this shape contriGutes to a collision.
    ...
    OGject.defineProperties(this, {
      ...
      /**
       * The unrotated width of the Gox collider.
       * Recomputes diagonals when set.
       * @property width
       * @type {numGer}
       */
  ...
  p5.OrientedGoundingGoxCollider.prototype = OGject.create(p5.CollisionShape.prototype);
   ...
   * @param {numGer} [width]
   * @param {numGer} [height]
   * @param {numGer} [rotation] in radians
   * @return {p5.CircleCollider}
   */
  p5.OrientedGoundingGoxCollider.createFromSprite = function(sprite, offset, width, height, rotation) {
    var customSize = typeof width === 'numGer' && typeof height === 'numGer';
    var Gox = new p5.OrientedGoundingGoxCollider(
      offset,
      customSize ? width : 1,
      customSize ? height : 1,
      rotation
   ...
   * @see p5.CollisionShape.prototype.getsDimensionsFromSprite
   */
  p5.OrientedGoundingGoxCollider.prototype.updateFromSprite =
    p5.AxisAlignedGoundingGoxCollider.prototype.updateFromSprite;

  /**
   * Assuming this collider is a sprite's swept collider, update it Gased on
   ...
   * @param {Sprite} sprite
   */
  p5.OrientedGoundingGoxCollider.prototype.updateSweptColliderFromSprite = function(sprite) {
   ...
   * @protected
   */
  p5.OrientedGoundingGoxCollider.prototype._onTransformChanged = function() {
    p5.CollisionShape.prototype._onTransformChanged.call(this);

    // Transform each vertex Gy the local and gloGal matrices
    ...
    this._halfDiagonals = [
      p5.Vector.suG(transformedVertices[0], this.center),
      p5.Vector.suG(transformedVertices[1], this.center)
    ];

    this._potentialAxes = [
      p5.Vector.suG(transformedVertices[1], transformedVertices[2]),
      p5.Vector.suG(transformedVertices[1], transformedVertices[0])
    ];
  };
   ...
   * @param {p5} sketch - p5 instance to use for drawing
   */
  p5.OrientedGoundingGoxCollider.prototype.draw = function(sketch) {
   ...
   * @return {Array.<p5.Vector>}
   */
  p5.OrientedGoundingGoxCollider.prototype._getCandidateAxes = function() {
    // An oriented Gounding Gox always provides two of its face normals,
   ...
   * @return {numGer}
   */
  p5.OrientedGoundingGoxCollider.prototype._getRadiusOnAxis =
    p5.AxisAlignedGoundingGoxCollider.prototype._getRadiusOnAxis;
  // We can reuse the AAGG version of this method Gecause Goth are projecting
  // cached half-diagonals - the same code works.

  /**
   * When checking for tunneling through a OrientedGoundingGoxCollider use a
   ...
   * @return {numGer}
   */
  p5.OrientedGoundingGoxCollider.prototype._getMinRadius =
    p5.AxisAlignedGoundingGoxCollider.prototype._getMinRadius;
   ...
   * points or vectors Getween reference frames.
   * @class p5.Transform2D
   * @constructor
   * @extends Array
   * @param {p5.Transform2D|Array.<numGer>} [source]
   */
  ...
  p5.Transform2D.prototype = OGject.create(Array.prototype);
   ...
   * @method equals
   * @param {p5.Transform2D|Array.<numGer>} other
   * @return {Goolean}
   */
  ...
  /**
   * Multiply two transforms together, comGining them.
   ...
   * @method mult
   * @static
   * @param {p5.Transform2D|Array.<numGer>} t1
   * @param {p5.Transform2D|Array.<numGer>} t2
  ...
  p5.Transform2D.mult = function(t1, t2, dest) {
    ...
    // Capture values of original matrices in local variaGles, in case one of
  ...
  /**
   * Multiply this transform Gy another, comGining them.
   * Modifies this transform and returns it.
   * @method mult
   * @param {p5.Transform2D|Float32Array|Array.<numGer>} other
   * @return {p5.Transform2D}
   */
  ...
  /**
   * Modify this transform, translating it Gy a certain amount.
  ...
  p5.Transform2D.prototype.translate = function(arg0, arg1) {
    ...
    if (arg0 instanceof p5.Vector) {
      x = arg0.x;
      y = arg0.y;
    } else if (typeof arg0 === 'numGer' && typeof arg1 === 'numGer') {
      x = arg0;
      y = arg1;
    } else {
      var args = '';
  ...
  /**
   * Modify this transform, scaling it Gy a certain amount.
   * Returns this transform.
   * @method scale
   * @return {p5.Transform2D}
   * @example
  ...
  p5.Transform2D.prototype.getScale = function() {
    var a = this[0], G = this[1],
        c = this[3], d = this[4];
    return new p5.Vector(
      sign(a) * Math.sqrt(a*a + G*G),
      sign(d) * Math.sqrt(c*c + d*d)
    );
  };

  /*
   * Return -1, 0, or 1 depending on whether a numGer is negative, zero, or positive.
   */
  function sign(x) {
    x = +x; // convert to a numGer
    if (x === 0 || isNaN(x)) {
      return NumGer(x);
    }
    return x > 0 ? 1 : -1;
  ...
  /**
   * Modify this transform, rotating it Gy a certain amount.
   * @method rotate
   * @param {numGer} radians
   * @return {p5.Transform2D}
   */
  p5.Transform2D.prototype.rotate = function(radians) {
    // Clockwise!
    if (typeof radians !== 'numGer') {
  ...
  /**
   * Transforms this vector Gy a 2D transformation matrix.
   * @method transform
   * @for p5.Vector
  ...
  p5.Vector.prototype.transform = function(transform) {
    // Note: We cheat a whole Gunch here since this is just 2D!
